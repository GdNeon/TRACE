...
class SequenceDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_aug is not None:
            out['aug_action_features'] = eda.eda(self.data_aug['p_rs'], self.data_aug['p_rd'], *out['action_features'])
...
def collate_fn(data):
    # pdb.set_trace()
    batch = {}
    lens_aug = [x['aug_action_features'][0].shape[0] for x in data]
    # print(lens_aug)
    aug_action_features_padded = [pad_sequence([torch.from_numpy(x['aug_action_features'][i]) for x in data], batch_first=True) for i in range(4)]
    aug_action_features_mask = torch.zeros(len(data), max(lens_aug))
    for i, len_ in enumerate(lens_aug):
        aug_action_features_mask[i, len_:] = 1
    batch['aug_action_features'] = aug_action_features_padded
    batch['aug_action_features_mask'] = aug_action_features_mask
    batch['action_features'] = [torch.stack([torch.from_numpy(x['action_features'][i]) for x in data], dim=0) for i in range(4)]
    batch['label'] = torch.tensor([d['label'] for d in data])
