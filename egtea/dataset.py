...
def read_representations(frames_A, frames_B, frames_C, frames_D, env, tran=None):
    ...
    for i in range(len(frames_A)):
        ...
        with env.begin() as e:
            dd = e.get(frames_A[i].strip().encode('utf-8')) or e.get(frames_B[i].strip().encode('utf-8')) or e.get(frames_C[i].strip().encode('utf-8')) or e.get(frames_D[i].strip().encode('utf-8'))
        if dd is None:
            print(frames_A[i])
...
def read_data(split_num, frames_A, frames_B, frames_C, frames_D, env, tran=None):
        ...
        for i, e in enumerate(env):
            if i == 0:
                ...
                for j in range(len(frames_A)):
                    rgb_frames_A.append(split_num + '_' + frames_A[j])
                    rgb_frames_B.append(split_num + '_' + frames_B[j])
                    rgb_frames_C.append(split_num + '_' + frames_C[j])
                    rgb_frames_D.append(split_num + '_' + frames_D[j])
                l.append(read_representations(rgb_frames_A, rgb_frames_B, rgb_frames_C, rgb_frames_D, e, tran))
            else:
                l.append(read_representations(frames_A, frames_B, frames_C, frames_D, e, tran))
        return l
    
    else:
        # otherwise, just read the representations
        return read_representations(frames_A, frames_B, frames_C, frames_D, env, tran)
...
class SequenceDataset(data.Dataset):
    def __init__(self, path_to_lmdb, path_to_csv, label_type='action',
        ...
        self.action_frames_A = []  # names of frames sampled from each action
        self.action_frames_B = []
        self.action_frames_C = []
        self.action_frames_D = []
    ...
    def __populate_lists(self):
        ...
        for _, a in tqdm(self.annotations.iterrows(), 'Populating Dataset', total=len(self.annotations)):
            ...
            if self.action_samples:
                ...
                action_frames_A = np.linspace(a.start, a.end, self.action_samples + 1, dtype=int)[1:]
                action_frames_B = np.linspace(a.start, a.end + 1, self.action_samples + 1, dtype=int)[1:]
                action_frames_C = np.linspace(a.start + 1, a.end, self.action_samples + 1, dtype=int)[1:]
                action_frames_D = np.linspace(a.start + 1, a.end + 1, self.action_samples + 1, dtype=int)[1:]
            ...
            if frames.min() >= 1:  # if the smaller frame is at least 1, the sequence is valid
                ...
                if self.action_samples:
                    self.action_frames_A.append(self.__get_frames(action_frames_A, a.video))
                    self.action_frames_B.append(self.__get_frames(action_frames_B, a.video))
                    self.action_frames_C.append(self.__get_frames(action_frames_C, a.video))
                    self.action_frames_D.append(self.__get_frames(action_frames_D, a.video))
    ...
    def __getitem__(self, index):
        ...
        if self.action_samples:
            # get action frames
            action_frames_A = self.action_frames_A[index]
            action_frames_B = self.action_frames_B[index]
            action_frames_C = self.action_frames_C[index]
            action_frames_D = self.action_frames_D[index]
        ...
        if self.action_samples:
            ...
            out['action_features'] = read_data(self.split_num, action_frames_A, action_frames_B, action_frames_C, action_frames_D, self.env, self.transform)
